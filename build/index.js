...
/*!*******************!*\
  !*** ./src/db.js ***!
  \*******************/
/*! exports provided: users, generateId */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
const resolvers = {
  Query: {
    user: (parent, {
      id
    }, context, info) => _db__WEBPACK_IMPORTED_MODULE_1__["users"].find(user => user.id == id),
    users: (parent, args, context, info) => _db__WEBPACK_IMPORTED_MODULE_1__["users"]
  },
  Mutation: {
    createUser: (parent, {
      name,
      ...
      const newUser = {
        id: Object(_db__WEBPACK_IMPORTED_MODULE_1__["generateId"])(),
        name,
        email,
        age
      };
      _db__WEBPACK_IMPORTED_MODULE_1__["users"].push(newUser);
      pubsub.publish(EVENT.USER_CREATED, {
        [EVENT.USER_CREATED]: newUser
      });
      return newUser;
    },
    updateUser: (parent, {
      ...
      const updatedUser = _db__WEBPACK_IMPORTED_MODULE_1__["users"].find(user => user.id == id);
      updatedUser.name = name;
      updatedUser.email = email;
      updatedUser.age = age;
      pubsub.publish(EVENT.USER_UPDATED, {
        [EVENT.USER_UPDATED]: updatedUser
      });
      return updatedUser;
    },
    deleteUser: (parent, {
      ...
      const userIndex = _db__WEBPACK_IMPORTED_MODULE_1__["users"].findIndex(user => user.id == id);

      if (userIndex === -1) {
        throw new Error('User not found');
      }

      const [deletedUser] = _db__WEBPACK_IMPORTED_MODULE_1__["users"].splice(userIndex, 1);
      pubsub.publish(EVENT.USER_DELETED, {
        [EVENT.USER_DELETED]: deletedUser
      });
      return deletedUser;
    }
  },
